...
func DeleteRepository(ownerID, repoID int64) error {
	// Call the RPC procedure for `DeleteRepository`
	args := &rpc_shared.Repo_DeleteRepositoryArgs{OwnerID: ownerID, RepoID: repoID}
	var reply interface{}

	err := rpc_client.Repo_DeleteRepository(args, &reply)
	if err != nil {
		return err
	}
	log.Info("RPC deleted repository!")
